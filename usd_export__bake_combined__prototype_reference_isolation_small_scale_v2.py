...
Script version: v2 (v1 crashed under forced xvfb/headful mode:
"AttributeError: 'Context' object has no attribute 'active_object'" --
bpy.context.active_object is unavailable early in an xvfb-launched,
non-`--background` context. Fixed here by reading via
bpy.context.view_layer.objects.active instead, matching the write-side
pattern already established for the bake-poll() fix in a prior session.)
