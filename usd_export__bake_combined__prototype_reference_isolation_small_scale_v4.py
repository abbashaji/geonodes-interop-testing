...
Script version: v4 (v1 crashed on active_object AttributeError -- fixed
in v2. v2 crashed on geometry_node_bake_single.poll() "missing active
object" -- fixed in v3 via temp_override(active_object=...). v3 got past
the bake but then crashed on wm.usd_export.poll() "context is incorrect"
-- a different failure signature, indicating wm.usd_export specifically
needs a real window/screen in its poll context, not just active-object
state. Fixed here by adding window=bpy.context.window_manager.windows[0]
and screen=window.screen to the export call's temp_override.)
...
def export_cell(obj):
    ...
    # First attempt (v3) used active_object/selected_objects/object alone,
    # which was sufficient for the bake operator but NOT for wm.usd_export --
    # it failed with "poll() failed, context is incorrect", a different
    # signature than the bake's "missing active object". wm.* operators
    # often specifically require a real window/screen in context, not just
    # active-object state. Supply both here.
