...
v2: fixed geometry_node_bake_single call to pass bake_id, not a list index.
v3: v2 completed without crashing but produced zero prims entirely -- Object
Info node never had "As Instance" enabled, so nothing was valid Instance-on-
Points prototype geometry. Fixed here. Also added a view_layer.update() call
immediately before the bake operator to test whether that closes the
separately-documented disk-bake-falls-back-to-packed gap.
"""
...
def build_scene():
    ...
    with _ctx_override():
        bpy.ops.object.geometry_node_bake_single(session_uid=base.session_uid, modifier_name=mod.name, bake_id=bake_entry.bake_id)
