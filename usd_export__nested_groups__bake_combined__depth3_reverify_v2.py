"""
usd_export__nested_groups__bake_combined__depth3 -- RE-VERIFICATION (v2)

This is a re-verification of the ORIGINAL usd_export__nested_groups__
bake_combined__depth3 result, not a new cell. The original script set
`mod.bakes[0].directory` (a per-bake-item field) to route the outer-level
bake to disk, and never checked whether that actually worked -- it only
trusted the operator's `{'FINISHED'}` return value. A later production-
scale sibling of this cell hit a harness bug where that exact field
silently falls back to a PACKED bake instead of a disk one (Blender logs
"Cannot determine bake location on disk. Falling back to packed bake."),
with every downstream export assertion still passing regardless -- because
none of them inspect the bake mechanism itself. This script is otherwise
IDENTICAL to the original (same depth=3, same 6x4x5=120-cube small scale,
same node construction) except: (1) bake routing uses the correct
MODIFIER-level `mod.bake_target`/`mod.bake_directory` fields, and (2) the
bake is independently verified via real on-disk cache JSON
(meta_dir_exists, instance count, referenced mesh vert count) rather than
trusting FINISHED alone. Settles whether the original's CONFIRMED WORKING
verdict actually exercised the disk-bake recovery-workaround path it
claimed to.
...
CONFIG = {
    "cell_id": "usd_export__nested_groups__bake_combined__depth3_reverify_v2",
    "maps_to_source": (
...
def trigger_bake(obj, mod, bake_node, bake_dir):
    ...
    import os
    # FIX (see HARNESS NOTES): the ORIGINAL script set bake_cfg.directory
    # (a per-bake-item field), which does NOT route to disk -- it silently
    # falls back to packed. The field that actually works is the
    # MODIFIER-level mod.bake_directory.
    ...
    # FIX: independently verify via real on-disk cache JSON instead of
    # trusting the operator's FINISHED result alone -- this is exactly the
    # check the original script omitted.
    cache_info = {
