...
Script version: v3 (v1 crashed on bake poll() context; v2 crashed on an
exact-count grid off-by-one before build_scene() finished -- both fixed here.
See HARNESS NOTES for both root causes.)
...
CONFIG block, per this session's harness-note lookup):
  ...
  This script (v3) attempts a DIFFERENT fix: inject genuine, distinct non-zero
...
CONFIG = {
    ...
    "maps_to_source": (
        ...
        "instances). Fix under test: non-flat micro-motion padding instead of "
    ...
    "output_path": "/tmp/gn_interop_scale_fix_attempt_v2_v3",
    "bake_directory": "/tmp/gn_interop_scale_fix_attempt_v2_v3_bake",
...
def build_scene():
    ...
    # x_subdivisions/y_subdivisions produce (N+1) vertices per axis, not N --
    # see HARNESS NOTES: solve for subdivisions=axis_count-1 to hit an exact total.
    bpy.ops.mesh.primitive_grid_add(x_subdivisions=199, y_subdivisions=99, size=50)
